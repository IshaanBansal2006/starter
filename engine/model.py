...
class Plan:
    ...
    def _warm_table(self, h: torch.Tensor, chunk: int = 2048) -> None:
        ...
        m = self.model
        B, T = self.B, self.T
        tail = min(T, int(os.environ.get("ENGINE_WARM_TAIL", "1024")))  # most recent context feeds the drafts
        if tail <= 0:
            return
...
class VerifyPlan:
    ...
    def verify(self) -> torch.Tensor:
        ...
        if self.recycler is not None:
            cand = self.recycler.update(self.blk.view(-1), logits).view(B, R)
            self.recycler.child_logit.copy_(self.recycler.child_logits(logits))
            return cand
